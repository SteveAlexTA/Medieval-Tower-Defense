...
void CannonTower::Update(std::vector<Enemy*>& enemies) {
    if (!enemies.empty()) {
        fireRate++;
        ...
        if (m_level == TowerLevel::LEVEL1) {
            fireThreshold = 300;  // Slow firing rate
        }
...
void CannonTower::shoot(std::vector<Enemy*>& enemies) {
    if (enemies.empty()) return;

    for (auto& enemy : enemies) {
        if (enemy && enemy->isAlive() && isInRange(enemy)) {
            CannonProjectile* projectile = new CannonProjectile(x + 16, y + 16, enemy, renderer);
...
CannonProjectile::CannonProjectile(int x, int y, Enemy* target, SDL_Renderer* renderer)
    : x(x), y(y), target(target), renderer(renderer), speed(5) {
